...
type DataStream = PassThrough & {
  byteLength: number;
  sizeExceeded: boolean;
}

type OnCommandType = (data: Buffer, next?: () => void) => void;
...
export class SMTPStream extends Writable {
  ...
  private _dataStream: DataStream | null;
  private _maxBytes: number;
  public dataBytes: number;
  private _continueCallback: Function | boolean;
  private _remainder: string;
  private _lastBytes: Buffer | false;
  public closed: boolean;

  public oncommand!: OnCommandType;
  ...
  startDataMode(maxBytes) {
    ...
    this.dataBytes = 0;
    this._dataStream = new PassThrough() as DataStream;
  ...
  _feedDataStream(chunk, done) {
    ...
    for (i = 2; i < len - 2; i++) {
      ...
      if (chunk[i] === 0x2e && chunk[i - 1] === 0x0a) {
        ...
        if (chunk[i + 1] === 0x2e) {
          ...
          if (this._dataStream && this._dataStream.writable) {
    ...
    if (this._lastBytes && this._lastBytes.length < chunk.length) {
